...
export type { DropPosType } from './components/DropAnchor/util'
            ...
            if (this.state.currentSelectId) {
                ...
                this.setState({
                    ...
                    selectComponentInstances: [...instanceList].filter((el) => {
                        ...
                        const ins = this.designRenderRef.current?.renderRef?.current?.dynamicComponentInstanceMap.get(
                            el._NODE_ID,
                        )
                        if (ins) {
                            res = ins._CONDITION
        ...
        ;['click', 'mouseover', 'mousedown', 'mouseup', 'mousemove'].forEach((ev: any) => {
            this.eventExposeHandler.push(
                addEventListenerReturnCancel<'click'>(
                    ...
                    ev,
                    async (e) => {
                        const targetComponentInstance = this.designRenderRef.current?.getInstanceByDom(
