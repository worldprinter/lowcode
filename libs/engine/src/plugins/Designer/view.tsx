...
import type { LayoutDragAndDropExtraDataType, LayoutDragEvent, LayoutPropsType } from '@worldprinter/lowcode-layout'
import { Layout } from '@worldprinter/lowcode-layout'
/**
 * TODO 注释 CSS 导入替换其他方式
 * import '@worldprinter/lowcode-layout/dist/style.css';
 */
import type { AssetPackage, CNode, CPage, CRootNode, InsertNodePosType } from '@worldprinter/lowcode-model'
...
export class Designer extends React.Component<DesignerPropsType, DesignerStateType> {
    ...
    onHoverNode = (node: CNode | CRootNode | null, startNode: CNode | CRootNode, event: MouseEvent) => {
        ...
        if (!startNode) {
            this.setState({
                hoverToolBar: (
                    <div className={styles.hoverTips}>{material?.value.title || material?.componentName}</div>
                ),
                ghostView: null,
            })
            return
