...
import type { BaseDragAndDropEventType } from '../../types/dragAndDrop'
import type { Pointer } from './common'
import type { SensorEventObjType, SensorEventType } from './sensor'
    ...
    registerSensor(sensor: Sensor) {
        ...
        const onMouseMove = async (mouseMoveEventObj: SensorEventType['onMouseMove']) => {
            ...
            if (this.currentState === 'NORMAL') {
                ...
                const isShaken =
                    Math.pow(pointer1.y - pointer2.y, 2) + Math.pow(pointer1.x - pointer2.x, 2) > SHAKE_DISTANCE
                // 小于抖动距离，不是拖拽
                if (!isShaken) {
                    return
                }
